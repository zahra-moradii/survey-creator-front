...
@Component({
  selector: 'app-profile-surveys',
  templateUrl: './profile-surveys.component.html',
  styleUrls: ['./profile-surveys.component.scss']
})
export class ProfileSurveysComponent implements OnInit {

  allSurveys: any | object = [];
  allUsers: any | object = [];
  len: any;
  codeValue: any;
  ...
  ngOnInit(): void {
    this.get.getAllFormsByProfileId().subscribe(
      (res: any) => {
        this.allSurveys = res.data;
        this.allSurveys.reverse();
        console.log("all surveys", this.allSurveys);
        this.allSurveys.forEach((el: any) => {
          this.getOne.getOneUser(el.userId).subscribe(
            (res: any) => {
